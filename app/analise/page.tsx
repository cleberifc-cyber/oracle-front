  ...
  const processarAcesso = async () => {
    ...
    try {
      ...
      if (res.ok && data.init_point) {
        ...
        alert("ERRO INFINITEPAY: " + (data.detail || "Verifique sua InfiniteTag no motor."));
      }
    } catch (err) {
      alert("Falha de rede com o terminal Oracle.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-[#0d1117] text-[#e6edf3] font-sans p-6">
      <div className="max-w-6xl mx-auto space-y-10">
        <header className="bg-[#161b22] p-6 rounded-3xl border border-[#30363d] flex justify-between items-center shadow-xl">
          ...
          <div className="text-[10px] text-[#8b949e] font-bold uppercase tracking-widest">Gemini 3 Ultra Engine</div>
        </header>
        ...
        {!acessoLiberado && !imagemFinal && !loading && (
          <div className="bg-[#161b22] border border-[#30363d] rounded-[48px] p-20 text-center space-y-8 shadow-2xl">
            <h2 className="text-4xl font-black text-white italic">LIBERAR ACESSO</h2>
            <div className="max-w-sm mx-auto space-y-4">
              <input type="text" placeholder="CUPOM (OPCIONAL)" value={cupom} onChange={(e)=>setCupom(e.target.value)} className="w-full bg-[#0d1117] border border-[#30363d] p-5 rounded-2xl text-center text-white outline-none focus:border-[#0070f3]" />
              <button onClick={processarAcesso} className="w-full bg-[#0070f3] hover:bg-[#005bc1] text-white font-black py-5 rounded-2xl transition-all shadow-[0_10px_20px_rgba(0,112,243,0.3)]">ADQUIRIR AGORA</button>
              <p className="text-[10px] text-gray-600 font-bold uppercase">Pagamento via InfinitePay</p>
            </div>
          </div>
        )}

        {loading && <div className="text-center py-20 animate-pulse text-[#0070f3] font-black tracking-widest text-xs">PROCESSANDO OPERAÇÃO...</div>}
        ...
        {acessoLiberado && !imagemFinal && !loading && (
          <div className="bg-[#161b22] border border-[#30363d] rounded-[48px] p-20 flex flex-col items-center justify-center relative cursor-pointer group shadow-2xl hover:border-[#0070f3]/50 transition-all">
            ...
            <p className="text-[#8b949e] text-sm mt-2">Arraste seu gráfico para análise institucional.</p>
          </div>
        )}
          ...
          <div className="space-y-8 pb-20 animate-in zoom-in duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-[#161b22] p-10 rounded-[40px] border border-[#30363d] shadow-xl">
                <p className="text-[10px] text-[#8b949e] uppercase font-black tracking-widest mb-2">Veredito</p>
              ...
              <div className="md:col-span-2 bg-[#161b22] p-10 rounded-[40px] border border-[#30363d] shadow-xl">
                <p className="text-[10px] text-[#0070f3] uppercase font-black tracking-widest mb-4">Laudo Gemini 3 Ultra</p>
                <p className="text-[#e6edf3] text-lg italic bg-[#0d1117] p-6 rounded-2xl border border-[#30363d]">"{analise.justificativa}"</p>
